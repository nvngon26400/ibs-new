...
import com.sbisec.helios.ap.brokerageMenu.wholeCustomer.dto.IfaKnockOutBrandHoldingListA002DtoResponseKnockOutBrandHoldingList;
import com.sbisec.helios.ap.brokerageMenu.wholeCustomer.dto.IfaKnockOutBrandHoldingListA004aDtoRequest;
import com.sbisec.helios.ap.brokerageMenu.wholeCustomer.service.IfaKnockOutBrandHoldingListService;
...
@Component(value = "cmpIfaKnockOutBrandHoldingListService")
public class IfaKnockOutBrandHoldingListServiceImpL implements IfaKnockOutBrandHoldingListService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(IfaKnockOutBrandHoldingListServiceImpL.class);
    ...
    public DataList<IfaKnockOutBrandHoldingListA002DtoResponse> displayA002(IfaKnockOutBrandHoldingListA002DtoRequest dtoReq)
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("IfaKnockOutBrandHoldingListServiceImplL.displayA002");
        }
        ...
        selSql001Req.setIsKnockIn(false); // ノックアウト用のSQL生成
        ...
        // レスポンス項目のセット
        List<IfaKnockOutBrandHoldingListA002DtoResponseKnockOutBrandHoldingList> knockOutBrandHoldingList = new ArrayList<>(); 
        for (IfaKnockInKnockOutBrandHoldingListSql001ResponseModel sqlResponse : selSql001Res.getDataList()) {
            IfaKnockOutBrandHoldingListA002DtoResponseKnockOutBrandHoldingList dtoResponse = new IfaKnockOutBrandHoldingListA002DtoResponseKnockOutBrandHoldingList();
            BeanUtils.copyProperties(dtoResponse, sqlResponse);
            knockOutBrandHoldingList.add(dtoResponse);       
        }

        IfaKnockOutBrandHoldingListA002DtoResponse response = new IfaKnockOutBrandHoldingListA002DtoResponse();
        response.setKnockOutBrandHoldingList(knockOutBrandHoldingList);
        
        // SQL001.総件数の取得
        int totalRow = selSql001Res.get(0).getTotalRow();

        DataList<IfaKnockOutBrandHoldingListA002DtoResponse> dtoRes = new DataList<IfaKnockOutBrandHoldingListA002DtoResponse>();
     ...
     * @exception exception システムエラー
     */
    public DataList<String> getPdfA004a(IfaKnockOutBrandHoldingListA004aDtoRequest dtoReq) throws Exception {

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("IfaKnockOutBrandHoldingListServiceImplL.getPdfA004a");
        }
