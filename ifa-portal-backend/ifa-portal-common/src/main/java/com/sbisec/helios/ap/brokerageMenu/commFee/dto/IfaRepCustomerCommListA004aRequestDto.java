package com.sbisec.helios.ap.brokerageMenu.commFee.dto;

import java.util.List;

import lombok.Data;

/**
 * 担当顧客別手数料一覧CSV出力リクエストDto.
 *
 * @author 宇田川達弥
 */
@Data
public class IfaRepCustomerCommListA004aRequestDto {
    
    /** 仲介業者コード（数字）. */
    private List<String> brokerCode;
    
    /** 仲介業者除外（半角英数字）. */
    private String chkBrokerCodeExclude;
    
    /** 支店コード（数字）. */
    private String branchCode;
    
    /** 営業員コード（半角英数字）. */
    private String empCode;
    
    /** 部店コード（半角英数字）. */
    private String butenCode;
    
    /** 口座番号（数字）. */
    private String accountNumber;
    
    /** 顧客名（漢字／カナ）（全角半角）. */
    private String customerNameKanjiKana;
    
    /** 顧客名（漢字／カナ）_条件. */
    private String customerNameKanjiKanaTerms;

    /** 取引コース（全角半角）. */
    private List<IfaRepCustomerCommListMultiSelectItem> course;
    
    /** 銘柄コード（半角英数字）. */
    private String brandCode;
    
    /** 集計単位. */
    private String chargeCustomerCountingUnit;
    
    /** 期間指定(From). */
    private String periodYmFrom;
    
    /** 期間指定(To). */
    private String periodYmTo;
    
    /** 証券種別. */
    private List<IfaRepCustomerCommListMultiSelectItem> securityClass;
    
}
