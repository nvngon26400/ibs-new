 ...
 * @author SBIえん
    2024/12/10 新規作成
*/
public class IfaJointSubscriptSecurityCashBalanceLookupCsvOut extends CsvOutPutUtil {
    
    public IfaJointSubscriptSecurityCashBalanceLookupCsvOut(ComplianceService comp) {
    ...
    public static String getHeaders(String pattern) {
        ...
        strCsvHead.append(CSV_SEPARATER);
        strCsvHead.append(DOUBLE_QUOTATION);
        strCsvHead.append("共募支店コード");
        strCsvHead.append(DOUBLE_QUOTATION);
        strCsvHead.append(CSV_SEPARATER);
        strCsvHead.append(DOUBLE_QUOTATION);
        strCsvHead.append("共募支店名");
        strCsvHead.append(DOUBLE_QUOTATION);
        strCsvHead.append(CSV_SEPARATER);
        strCsvHead.append(DOUBLE_QUOTATION);
    ...
    /**
     * 共同募集　証券・金銭・残高データをcvs形式に変換
     *
     * @param entrustInvestment 共同募集　証券・金銭・残高データ
     * @return String cvs形式共同募集　証券・金銭・残高データ
     */
    @Override
    protected String getCsvLine(ModelBase modelBase, String pattern) {
        
    	IfaJointSubscriptSecurityCashBalanceLookupCsvItems csvItems =
                (IfaJointSubscriptSecurityCashBalanceLookupCsvItems) modelBase;
        ...
        // 共募支店コード（数字）
        strCsv.append(DOUBLE_QUOTATION);
        strCsv.append(StringUtil.nullToEmpty(csvItems.getJointBranchCode()));
        strCsv.append(DOUBLE_QUOTATION);
        strCsv.append(CSV_SEPARATER);
        // 共募支店名（全角半角）
        strCsv.append(DOUBLE_QUOTATION);
        strCsv.append(StringUtil.nullToEmpty(Cp932.toJIS(csvItems.getJointBranchName())));
        strCsv.append(DOUBLE_QUOTATION);
        strCsv.append(CSV_SEPARATER);
        // 証券種別名
        strCsv.append(DOUBLE_QUOTATION);
