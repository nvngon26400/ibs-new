  ...
  <div :class="{ 'collapse': collapse }">
    <transition>
      ...
      >
        ...
        <span class="sub-el-title"><el-image class="sss-icon" :src="require('@/assets/icons/sss/SSS_logo.svg')"/>(債券メニュー)</span>
      </div>
    </transition>
    ...
    >
        ...
        <div v-if="errorFlg === 'error'">
          <el-row>
            <el-col class="errorMessage">
              <span>SSS(債券メニュー) 接続の処理でエラーが発生したため、画面を表示できません。</span>
            </el-col>
          </el-row>
          <el-row>
      ...
      <form ref="sssForm" @submit.prevent="onSubmit" :action="sssLoginUrl" method="GET" target="_blank">
        <input type="hidden" name="code" :value="sssLoginParam" />
      </form>
      <ifa-requester
        id="ifaSssLoginInitializeA001"
...
export default {
  ...
  methods: {
    ...
    ifaSssLoginInitializeA001() {
      this.$nextTick(() => {
        document.getElementById('ifaSssLoginInitializeA001').click();
      });
    },
