...
<script>
import { getPlaceholderString, monthFormat } from './js/IfaMonthPickerFunction'
...
export default {
  ...
  data() {
    return {
      internalValue: '',
      dispFormat: monthFormat(this.domain.format),
  ...
  methods: {
    ...
    handleBlur(currentInstance) {
      ...
      this.dispFormat = monthFormat(this.domain.format)
