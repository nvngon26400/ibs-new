...
export default {
  name: 'IfaDateTimePicker',
  props: {
    id: { type: String, required: false, default: null }, // ID
    modelValue: { type: [String, Number], required: false, default: '' }, // 入力値(v-model用)
  ...
  methods: {
    handleFocus() {
      // 編集時はYYYYMMDD HHmmssのフォーマットとする。
      this.dispFormat = 'YYYYMMDD HHmmss'

      // 入力開始時にフォームバリデーションを無効にし、値削除後の即時バリデートエラーを抑止する。
      this.validateEventFlag = false
    },
    handleChange(value) {
      const formattedDisplayedValue = this.$_getFormattedDateTimeValue(value, this.domain.format)
...
.form_label :deep(.el-form-item__content) {
  --ifa-input-validation-error-width: 265px;
}
