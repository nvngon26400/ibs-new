...
export default {
  ...
  computed: {
    rateDomain() {
      let digitSpec = 2 // デフォルト値
      if (this.digit && !isNaN(this.digit) && this.digit !== '' && this.digit >= 0) {
        // プロパティで小数点以下桁数が指定されていたらプロパティの値を採用する
        digitSpec = this.digit
      } else if (this.domain && !isNaN(this.domain.digit) &&
          this.domain.digit !== '' && this.domain.digit >= 0) {
        // プロパティが指定なしならドメインの小数点以下桁数を採用する
        digitSpec = this.domain.digit
      }
      return {
