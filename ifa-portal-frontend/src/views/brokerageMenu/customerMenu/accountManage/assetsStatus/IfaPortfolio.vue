...
export default {
  ...
  computed: {
    portfolioSummary() {
      ...
      const dispalyOrder = [
        '国内株式', '国内債券', '投資信託', '外国債券(円建)', '外国株式', '外貨建MMF', '外国債券(外貨建)',
        'ST', '信用建玉', '米株信用建玉', '現金(円貨)', '現金(外貨)', '先OP(保証金等)'
      ]
    ...
    displayMessage() {
      if (!this.hasSummaryList() && !this.hasHoldingSecurityList() && this.form.jobStatus !== '1') {
        return this.form.noDetailMsg
      } else if (this.form.jobStatus === '1') {
        return this.form.jobRunningMsg
      }
      return null
    }
  },
  ...
  methods: {
    ...
    responseHandlerA001(response) {
      ...
      // 投資信託リストの表示順入れ替え
      // 国内投信 >  国内投信（SBIラップ投信） > 外国投信の順番になるように、リストを並び変える
      if (this.form.holdingSecurityListMutualFund) {
        this.form.holdingSecurityListMutualFund.sort((a, b) => {
          return mutualFundOrder[a.secTypeName] - mutualFundOrder[b.secTypeName];
        })
      }
