<template>
  <!-- SUB0202_0110-02_1その他余力拘束注文取消確認 -->
  <el-dialog
    v-model="vmIsVisible"
    ...
    <el-row style="margin-top: 20px;">
      ...
      >
        <ifa-button
          ...
          :request-model="ifaOtherRemainPowerRestrainCancelConfirmA002RequestModel"
          action-id="SUB0202_0110-02_1#A002"
...
import { IfaOtherRemainPowerRestrainCancelConfirmFormModel } from './js/IfaOtherRemainPowerRestrainCancelConfirmFormModel'
import { IfaOtherRemainPowerRestrainCancelConfirmA002RequestModel } from './js/IfaOtherRemainPowerRestrainCancelConfirmA002RequestModel'
...
export default {
  ...
  emits: ['close-modal', 'cancel-complete', 'update:isVisible'],
  setup(props) {
  ...
  data() {
    return {
      ...
      // 画面のフォームモデルを初期化
      form: new IfaOtherRemainPowerRestrainCancelConfirmFormModel(),
      messages: {
        mains: [],
        errors: [],
        warnings: [],
  ...
  computed: {
    ifaOtherRemainPowerRestrainCancelConfirmA002RequestModel() {
      return new IfaOtherRemainPowerRestrainCancelConfirmA002RequestModel(this.form)
    },
    customerName() {
  ...
  methods: {
    ...
    responseHandlerActionA002: function(data) {
      this.$emit('cancel-complete', data.dataList[0])
    },
    /**
