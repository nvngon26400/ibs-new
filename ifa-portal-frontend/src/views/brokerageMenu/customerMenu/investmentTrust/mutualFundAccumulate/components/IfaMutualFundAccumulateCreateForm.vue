<template>
          ...
          <el-row>
            ...
            <el-col :span="3">
              <div style="display: flex; align-items: center; justify-content: flex-end;">
                ...
                ></ifa-button>
                <ifa-button
                  v-if="info?.goBackButton === 'SUB0202_0401' || info?.goBackButton === 'SUB0202_0403'"
                  id="btnBack"
                  text="戻る"
                  color="secondary"
                  action-type="originalAction"
        ...
        <div class="form-area__section">
          <el-row>
            <el-col :span="20">
              ...
              >
                <template #label>
                  <span style="font-weight: 700;">預り区分</span>
                </template>
                <template v-if="info?.accountTypeOptions && info?.accountTypeOptions?.length === 1">
                  <span>{{ info?.accountTypeOptions?.[0]?.value ?? '-' }}</span>
                  <input
                ...
                <template v-else>
                  ...
                  >
                    <el-radio
                      v-for="option in info?.accountTypeOptions"
                      :key="option.key"
                      :label="option.key"
                    >
                      {{ option.value }}
            ...
            <el-row>
              ...
              <div>
                ...
                >
                  ...
                  >
                    <template #suffix>
                      ...
                      <span style="padding-left: 40px; color: grey">※1日・30日・月末はご選択できません。</span>
                    </template>
                  </ifa-input-select>
                </div>
                <div
                ...
                >
                  ...
                  <span style="color: grey">※1日・30日・月末はご選択できません。</span>
                </div>
                <div
  ...
  <ifa-mutual-fund-accumulate-confirm
    v-if="confirmVisible"
    :visible="confirmVisible"
    :confirm-info="confirmInfo"
    :confirm-payload="A010RequestModel"
    @on-cancel="confirmVisible = false"
    @on-redirect="handleRedirect"
  ></ifa-mutual-fund-accumulate-confirm>

  <ifa-requester
    id="ifaMutualFundAccumulateSettingInputConfirmA010"
    action-id="SUB0202_0403-02_1#A010"
    action-type="requestAction"
    :request-model="A010RequestModel"
    @response-handler="settingInputConfirmA010Handler($event)"
    @response-error-handler="settingInputConfirmA010ErrorHandler"
  ></ifa-requester>
</template>
...
import IfaMutualFundAccumulateConfirm from './IfaMutualFundAccumulateConfirm.vue'

import IfaMutualFundAccumulateInputCheckText from './IfaMutualFundAccumulateInputCheckText.vue'
import { IfaMutualFundAccumulateSettingInputConfirmA010RequestModel } from '../js/IfaMutualFundAccumulateSettingInputConfirmA010RequestModel'
import { IfaMutualFundAccumulateSettingInputFormModel } from '../js/IfaMutualFundAccumulateSettingInputFormModel'
...
export default {
  components: {
    ...
    IfaBalloonIcon,
    IfaRequester,
    IfaMutualFundAccumulateConfirm,
    IfaMutualFundAccumulateInputCheckText
  },
  props: {
    info: { type: Object, required: true }
  },
  emits: ['on-redirect', 'on-close'],
  data() {
    return {
      IfaYen80DomainModel,
      form: new IfaMutualFundAccumulateSettingInputFormModel(this.info),
  ...
  computed: {
    ...
    accountTypeRequired() {
      const moreThanOneAccountType = this.info?.accountTypeOptions && this.info?.accountTypeOptions?.length > 1
      const eitherNisaOrTaxShiftAccountType = this.info?.reserveOrderUnit === '0' && (this.form.accountType === 'H' || this.form.accountType === 'I')

      return moreThanOneAccountType || eitherNisaOrTaxShiftAccountType
    },
    minAppAmount() {
    ...
    availableAppSettingAmount() {
      const {
        ...
        oneYearSumAmount: { nisaReserveSettingAmount }
      } = this.info

      const accountTypeName = accountTypeOptions.find(v => v.key === this.form.accountType)?.value ?? '-'
      ...
      if (this.form.accountType === 'I') {
        ...
        const diffVal = nisaReserveSettingAmountVal.plus(bonusSumAmountNisaReserveVal)

        const diffValTotal = oneYearLimitNisaReserveAmountVal.minus(diffVal)
    ...
    numberOfApplications() {
      ...
      // 複数日
      if (this.form.courseType === '4') return `${this.form.settingReserveMultiDay.length}`
      // 奇数月 | 偶数月
    ...
    A010RequestModel() {
      return new IfaMutualFundAccumulateSettingInputConfirmA010RequestModel(this.form, this.info)
    }
  },
  mounted() {
    if (this.info?.accountTypeOptions && this.info?.accountTypeOptions?.length >= 1) {
      this.form.accountType = this.info?.accountTypeOptions?.[0]?.key
    }
  },
  methods: {
    handleReset() {
    ...
    handleClose() {
      if (this.info?.goBackButton === 'SUB0202_0401') {
        return this.$emit('on-redirect', {
          fundCodeTimes: this.info?.mfkaisu,
          fundCodeIssues: this.info?.mfgo,
          kyoukaiCd: this.info?.fundCode,
          listFlag: this.info?.listFlag,
          // from page id -> 購入積立可能一覧
          source: 'SUB0202_0403'
        })
      }
    ...
    handleValidate() {
      ...
      formEl.validate((valid) => {
        if (valid) {
          this.$nextTick(() => {
            document.getElementById('ifaMutualFundAccumulateSettingInputConfirmA010').click()
          })
        }
      })
    },
    ...
    settingInputConfirmA010Handler(res) {
      this.$_logDebug(res)

      this.confirmInfo = res?.dataList?.[0] ?? {}
      this.confirmVisible = true
    },
    settingInputConfirmA010ErrorHandler(error) {
