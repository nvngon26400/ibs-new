  ...
  computed: {
    ...
    innerSasinariHouhou: {
      ...
      set(newValue) {
        ...
        } else if (this.innerSasinariHouhou === '2' && newValue === '1') { // 成行 → 指値
          switch (this.innerSasinariJyouken) {
    ...
    // ストップ安
    stopLow() {
      if (this.tradeCd === '5' || this.tradeCd === '') {
    ...
    beforeCorrectPriceStr() {
      ...
      if (this.beforeCorrectPriceParam.sasinariHouhou === '1') {
        ...
        if (this.orderKind === '2' && this.beforeCorrectPriceParam.sasinariJyouken === ' ') { // OCO1 かつ 執行条件なしの場合
          sasinariJyoukenStr = ''
        } else if (this.beforeCorrectPriceParam.sasinariJyouken !== ' ') { // 執行条件ありの場合
      ...
      if (this.beforeCorrectPriceParam.sasinariHouhou === '3') {
        ...
        if (this.beforeCorrectPriceParam.triggerPriceText === ' ') { // 指定なしの場合、取引種別から以上/以下を取得
          switch (this.tradeCd) {
            case '5':
        ...
        } else if (this.beforeCorrectPriceParam.triggerPriceText === '0' || this.beforeCorrectPriceParam.triggerPriceText === '1') { // 指定ありの場合
          triggerPriceTextStr = this.$_out(this.$_getCodeValue('LATEST_TRIGGER_ZONE', 1, this.beforeCorrectPriceParam.triggerPriceText)) + 'になった時点で'
        }
        // 執行条件
        let sasinariJyoukenStr = ''
        ...
        return '現在値が' + triggerPriceStr + triggerPriceTextStr + sasinariJyoukenStr + priceStr + 'で執行'
      }
      return '-'
    },
