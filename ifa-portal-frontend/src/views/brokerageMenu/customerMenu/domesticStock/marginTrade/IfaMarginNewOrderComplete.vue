        ...
        >
          ...
          <div v-if="refFormModel.orderKind =='3' || refFormModel.orderKind =='4'">
            ...
            <el-row class="dotted_border">
              ...
              >
                <span>
                  {{ getOco2JyoukenPriceOther(refFormModel.oco2GyakusasiHouhou, refFormModel.oco2GyakusasiJyouken, refFormModel.oco2Price, refFormModel.oco2TriggerPrice) }}
                </span>
              </el-col>
              <!-- IFD-IFD2 -->
              <el-col
...
export default {
  ...
  methods: {
    ...
    getOco2JyoukenPriceOther(houhou, jyouken, price, trigger) {
      let tradeCdStr = this.refFormModel.tradeCd === '3' ? '6' : '5';
      return '現在値が' + this.$_addComma(trigger) + '円' +
    ...
    getIfd2JyoukenPrice(houhou, jyouken, price, trigger) {
      if (houhou === '1') { // 通常/逆指値.執行方法=指値
        ...
        let tradeCdStr = this.refFormModel.tradeCd === '3' ? '6' : '5';
        return '現在値が' + this.$_addComma(trigger) + '円' + (tradeCdStr === '5' ? '以上' : '以下') + 'になった時点で' +
