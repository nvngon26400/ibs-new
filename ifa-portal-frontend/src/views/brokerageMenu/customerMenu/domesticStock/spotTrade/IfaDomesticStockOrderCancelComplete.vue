  ...
  >
    ...
    >
      <el-col
        :offset="1"
      >
        <span>{{ $_out(customerInfo.butenCode) }}-{{ customerInfo.accountNumber ? customerInfo.accountNumber.padStart(7, "0") : "-" }}</span>
      </el-col>
    </el-row>
    <el-row
        ...
        >
          ...
          <el-row>
            ...
            >
              ...
              >
                ...
                <span v-else-if="!cancelData.orderKind">-</span>
                <ifa-text
                  v-else
            ...
            <el-row class="dotted_border">
              ...
              >
                <span
                  style="font-size: 16px;"
                >
                  {{ $_out($_withCommaInteger(cancelData.quantity)) }}株</span>
              </el-col>
            </el-row>
          </el-row>
          ...
          >
            ...
            <el-col
              :span="16"
            >
              <span v-if="cancelData.orderKind === '1' && !cancelData.sasinariHouhou"
                    style="font-size: 16px;"
              >-</span>
              <ifa-text
                v-else-if="cancelData.orderKind === '1'"
                code-list-id="EXECUTE_METHOD"
                :disp-pattern="1"
                :code-key="cancelData.sasinariHouhou"
                style="font-size: 16px;"
              ></ifa-text>
              <span v-if="(cancelData.orderKind === '3' || cancelData.orderKind === '4') && !cancelData.ifd1SasinariHouhou"></span>
              <ifa-text
                v-else-if="(cancelData.orderKind === '3' || cancelData.orderKind === '4')"
                code-list-id="EXECUTE_METHOD"
                :disp-pattern="1"
              ...
              ></ifa-text>
              <br>
              <span v-if="cancelData.orderKind === '1'"
                    style="font-size: 16px;"
              >
                {{ getPrice(priceParam, cancelData.tradeCd) }}</span>
              <span v-if="(cancelData.orderKind === '3' || cancelData.orderKind === '4')"
                    style="font-size: 16px;"
              >
                {{ getPrice(ifd1PriceParam, cancelData.tradeCd) }}</span>
            </el-col>
          </el-row>
          ...
          <el-row v-if="cancelData.orderKind === '2'">
            ...
            <el-row class="dotted_border">
              ...
              <el-col
                :span="16"
              >
                <span style="white-space: pre-wrap;">{{ getOco2Price(cancelData.tradeCd, cancelData.oco1SasinariJyouken) }}</span>
              </el-col>
            </el-row>
          </el-row>
          ...
          <el-row>
            ...
            >
              ...
              >
                <span
                  style="font-size: 16px;"
                >
                  {{ $_out($_getFormattedDateValue(cancelData.period)) }}</span>
              </el-col>
            </el-row>
            ...
            >
              ...
              <el-col
                :span="16"
              >
                <span v-if="!customerInfo.customerAttribute">-</span>
                <span v-else>
                  <ifa-text
        ...
        >
          ...
          >
            ...
            <el-col
              :span="16"
            >
              <span v-if="!cancelData.ifd2SasinariHouhou"
                    style="font-size: 16px;"
              >-</span>
              <ifa-text
                v-else
                ...
                style="font-size: 16px;"
              ></ifa-text>
              <br><span style="font-size: 16px;">{{ getPrice(ifd2PriceParam, '2') }}</span>
            </el-col>
          </el-row>
...
import { IfaDomesticStockOrderCancelCompleteFormModel } from './js/IfaDomesticStockOrderCancelCompleteFormModel'
...
export default {
  ...
  emits: ['to-order-status-list', 'update:isVisible'],
  setup(props) {
  ...
  data() {
    return {
      form: new IfaDomesticStockOrderCancelCompleteFormModel(),
      priceParam: {
        sasinariHouhou: '',
        sasinariJyouken: '',
        triggerPrice: '',
  ...
  methods: {
    onShow() {
      ...
      } else if (this.cancelData.tradeCd === '2') {
        // 売却
    ...
    toOrderStatusListA001() {
      this.$emit('to-order-status-list')
    },
    // 価格欄に表示する内容を生成する
    getPrice(param, tradeCd) {
      ...
      // 指値
        ...
        if (param.sasinariJyouken !== ' ') {
          return this.$_out(this.$_getCodeValue('LIMIT_MARKET_TYPE', 2, param.sasinariJyouken)) +
               '/' + str
        } else {
          return str
        }
      ...
      // 成行
        if (param.sasinariJyouken !== 'N') {
          return this.$_out(this.$_getCodeValue('LIMIT_MARKET_TYPE', 3, param.sasinariJyouken))
        } else {
          return null
        }
    ...
    getOco2Price(tradeCd, sasinariJyouken) {
      ...
      str += '\n' // 改行
      this.$_logDebug('*' + sasinariJyouken + '*')
      if (sasinariJyouken === ' ') {
      // 条件なし
        str += 'OCO1（指値）を'
      } else {
