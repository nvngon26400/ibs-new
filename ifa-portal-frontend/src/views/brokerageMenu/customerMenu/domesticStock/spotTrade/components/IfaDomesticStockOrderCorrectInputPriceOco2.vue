  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <ifa-input-price
            ...
            :disabled="workingStatus === 'true' && orderKind !== '4'"
            :min="minPrice"
            :max="maxPrice"
      ...
      >
        ...
        >
          <template
            v-if="priceParam.sasinariHouhou === '1' || priceParam.sasinariHouhou === '3'"
          >
            <span
              v-if="stopLow !== '1.0'
...
export default {
  ...
  computed: {
    ...
    initialStepPrice() {
      // 注文単価
      const price = Number(this.beforeCorrectPrice?.price)
    ...
    initialStepTrigger() {
      // 発火条件価格
      const triggerPrice = Number(this.beforeCorrectPrice?.triggerPrice)
  ...
  methods: {
    // 整数と小数で最大値が変化する可能性を考慮して、ドメインの最大値を取得する
    getMaxValueWithDomain(domain, value) {
      const precision = domain.precision // 最大桁数
      const digit = domain.digit // 最大小数点以下桁数

      // 整数の場合
      if (Number.isInteger(value) || digit === 0) {
        return '9'.repeat(precision)
      }

      // 小数の場合
      const decimal = '9'.repeat(precision - digit - 1) // 小数点の分追加で-1
      const fractional = '9'.repeat(digit)
