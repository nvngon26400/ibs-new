<template>
  <!-- 画面名：国内信用建玉期日アラート -->
  <div>
    <screen-title text="国内信用建玉期日アラート一覧"></screen-title>
    <el-form
      ref="form"
      :model="form"
      :inline="true"
            ...
            <div class="filter-container">
              ...
              >
                <ifa-common-search
                  ...
                  display-pattern="pt1"
                  list-pattern="pt2"
                  original-screen-id="SUB020301_02-01"
              ...
              <el-row style="padding-left: 46px; padding-top: 10px; display: block;">
                <ifa-button
                  ...
                  :request-model="ifaDomesticMarginPositionDueDateAlertA002RequestModel"
                  action-id="SUB020301_02-01#A002"
    ...
    <ifa-requester
      id="ifaDomesticMarginPositionDueDateAlertA002"
      action-type="requestAction"
      :request-model="ifaDomesticMarginPositionDueDateAlertA002RequestModelIfaWholePortalHome"
      action-id="SUB020301_02-01#A002"
      @response-handler="responseA002Handler"
      @response-error-handler="responseA002ErrorHandler"
    ></ifa-requester>
...
import { IfaDomesticMarginPositionDueDateAlertA002RequestModel } from './js/IfaDomesticMarginPositionDueDateAlertA002RequestModel'
...
export default {
  ...
  data() {
    return {
      formRef: {},
      domesticCreditOpenInterestDueDateAlert: [],
      form: {
        brokerCode: '',
  ...
  computed: {
    ifaDomesticMarginPositionDueDateAlertA002RequestModel() {
      return new IfaDomesticMarginPositionDueDateAlertA002RequestModel(this.form)
    },
    ifaDomesticMarginPositionDueDateAlertA002RequestModelIfaWholePortalHome() {
      return new IfaDomesticMarginPositionDueDateAlertA002RequestModel(this.serchFormIfaWholePortalHome)
    }
  },
  mounted() {
  ...
  methods: {
    onShow(resume, isRouting) {
      ...
      if (isRouting) {
        ...
        this.$nextTick(() => {
          document.getElementById('ifaDomesticMarginPositionDueDateAlertA002').click()
        })
      }
    },
    preSearch() {
      this.onShow()
      this.ifaDomesticMarginPositionDueDateAlertA002RequestModel.customerNameKanjiKanaTerms = this.form.customerNameKanjiKanaTerms - 1
    },
    ...
    handleClick({ rowData: { buten, accountNumber }, dataIndx }) {
      if (dataIndx === 'detail') {
        this.$_startShowMenu('SUB0202_010202', { butenCode: buten, accountNumber })
      }
    },
...
// 国内信用建玉期日アラート
const domesticCreditOpenInterestDueDateAlert = [
...
obj.colModel = domesticCreditOpenInterestDueDateAlert
