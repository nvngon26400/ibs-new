<template>
  <div>
          ...
          >
            ...
            <el-row>
              <ifa-input-text
                ...
                required
                style="width: 35rem;"
                original-screen-id="SUB0602-02_1"
    ...
    ></ifa-ok-cancel-dialog>
    <ifa-requester
      id="ifaPortalNotificationNewRegisterPortalNoticeNewRegistrationOkA007"
      action-id="SUB0602-02_1#A007"
...
import { IfaPortalNotificationNewRegisterA007RequestModel } from './js/IfaPortalNotificationNewRegisterA007RequestModel'
import { IfaPortalNotificationNewRegisterFormModel } from './js/IfaPortalNotificationNewRegisterFormModel'
...
export default {
  ...
  data() {
    return {
      ...
      form: new IfaPortalNotificationNewRegisterFormModel(),
  ...
  computed: {
    A007RequestModel() {
      return new IfaPortalNotificationNewRegisterA007RequestModel(this.form)
    },
    previousBusinessDay() {
  ...
  mounted() {
    this.formModel = new IfaPortalNotificationNewRegisterFormModel()
  ...
  methods: {
    ...
    doOperation() {
      ...
      this.form.period[0] = this.form.period && this.form.period[0] ? this.form.period[0].replace(/(\/)/g, '') : ''
      this.form.period[1] = this.form.period && this.form.period[1] ? this.form.period[1].replace(/(\/)/g, '') : ''
      document.getElementById('ifaPortalNotificationNewRegisterPortalNoticeNewRegistrationOkA007').click()
    },
    responseHandlerA007() {
      this.updateTable()
    },
    ...
    resetForm() {
      ...
      this.form.period = []
      this.form.notificationContent = ''
      this.form.linkAddress = ''
      this.form.importantNotification = false
      this.form.displayOrder = ''
      this.form.appearPeriod = []
      this.form.nonDisplay = false
    },
    // ご連絡内容項目にリンクを挿入
    setLink() {
      this.$nextTick(() => {
    ...
    switchTitleMessage() {
      this.title = '登録確認'
      this.message = '入力された内容で登録します。よろしいですか。'
    },
