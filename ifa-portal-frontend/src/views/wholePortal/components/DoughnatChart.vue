...
export default ({
  ...
  methods: {
    ...
    updateData() {
      // 評価額が未設定の商品を除外する
      const exSecurityClassList = ['信用建玉', '米株信用建玉']
      const portfolioSummaryList = this.chartDataForm.portfolioSummaryList.filter(item => !exSecurityClassList.includes(item.securityClass))

      this.datas.labels = portfolioSummaryList.map(item => this.$_out(item.securityClass))
      this.datas.datasets[0].data = portfolioSummaryList.map(
