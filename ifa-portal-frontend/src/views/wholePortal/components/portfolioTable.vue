  ...
  <div>
      ...
      >
        ...
        >
          <tr>
            ...
            <td
              class="_table__data"
              v-html="addLineBreakHintsForDepositType($_out($_getCodeValue('ASSET_SPECIFIC_DEPOSIT_TYPE', 1, item.depositType)))"
            ></td>
            <td
      ...
      >
        ...
        >
          <tr>
            ...
            <td
              class="_table__data"
              v-html="addLineBreakHintsForDepositType($_out(mutualFunddepositTypeName(item.depositType, item.secTypeName)))"
            ></td>
            <td class="_table__data _right">{{ $_out($_withCommaInteger(item.contractStandardDeposit)) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaNoneZeroPadding(item.openPrice, 7)) }}</td>
      ...
      >
        <thead>
          ...
          <tr>
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '16% !important' : '255px'}"
            >
              銘柄名
            </th>
            <th
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '7% !important' : '95px'}"
            >
              買付単価
            </th>
            <th
        ...
        >
          <tr>
            ...
            <td class="_table__data">{{ $_out($_getFormattedDateValue(item.redemptionDate, 'date8')) }}</td>
            <td class="_table__data">{{ $_out(breakAfterComma(item.interestPaymentDate)) }}</td>
            <td class="_table__data _right">{{ item.openPrice ? $_out($_withCommaNoneZeroPadding(item.openPrice, 7)) + item.currency : '-' }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaInteger(item.contractStandardDeposit)) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaNoneZeroPadding(item.valuation,4)) }}</td>
            <td class="_table__data _right">
              <span :class="[Number(item.profitAndLoss) < 0 ? 'font-color__minus' : $_out($_signedWithCommaInteger(item.profitAndLoss)) !== '-' && Number(item.profitAndLoss) >= 0 ? 'font-color__plus' : '', '__right bold']">
      ...
      >
        ...
        >
          <tr>
            ...
            <td
              class="_table__data"
              v-html="addLineBreakHintsForDepositType($_out($_getCodeValue('FOREIGN_DEPOSIT_TYPE', 5, item.depositType)))"
            ></td>
            <td class="_table__data _right">{{ $_out($_withCommaInteger(item.contractStandardDeposit)) }}</td>
            <td class="_table__data _right">{{ item.fxRate ? $_withCommaZeroPadding(item.fxRate) + '円/' + item.currency : '-' }}</td>
      ...
      >
        <thead>
          ...
          <tr>
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '7% !important' : '120px'}"
            >
              銘柄コード
            </th>
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '17% !important' : '245px'}"
            >
              銘柄名
            </th>
            <th
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '7% !important' : '100px'}"
            >
              償還日
            </th>
            <th
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '6.5% !important' : '95px'}"
            >
              保有額面
            </th>
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '9% !important' : '120px'}"
            >
              取得単価<br>「現地通貨」
            </th>
            <th
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '9% !important' : '105px'}"
            >
              評価額<br>「円」
            </th>
            <th
    ...
    <el-row
      v-if="portfolioData.holdingSecurityListCash && portfolioData.holdingSecurityListCash.length > 0 ||
        portfolioData.holdingSecurityListSweepAccountList && portfolioData.holdingSecurityListSweepAccountList.length > 0 ||
    ...
    >
      ...
      >
        ...
        <tbody>
          ...
          <tr
            v-for="(item, idx) in portfolioData.holdingSecurityListSweepAccountList"
            :key="idx"
          >
            <td class="_table__data"> {{ $_out(item.name) }}</td>
            <td class="_table__data _right">{{ $_out(item.fxRate) }}</td>
      ...
      >
        <thead>
          ...
          <tr>
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '5% !important' : '70px'}"
            >
              市場
            </th>
            <th
        ...
        >
          <tr>
            ...
            <td class="_table__data">{{ $_out(item.market) }}</td>
            <td class="_table__data">{{ $_getFormattedDateValue(item.lastTradeDate, 'date8') ? $_getFormattedDateValue(item.lastTradeDate, 'date8') : $_out(item.lastTradeDate) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaInteger(item.contractStandardDeposit)) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaNoneZeroPadding(item.openPrice, 7)) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaNoneZeroPadding(item.price, 7)) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaNoneZeroPadding(item.valuation,4)) }}</td>
          ...
          <tr>
            ...
            <td class="_table__data _right">
              <span :class="[Number(portfolioData.marginPositionProfitAndLossTotal) < 0 ? 'font-color__minus' : $_out($_signedWithCommaInteger(portfolioData.marginPositionProfitAndLossTotal)) !== '-' && Number(portfolioData.marginPositionProfitAndLossTotal) >= 0 ? 'font-color__plus' : '', '__right bold']">
                {{ $_out($_signedWithCommaNoneZeroPadding(portfolioData.marginPositionProfitAndLossTotal)) }}
              </span>
            </td>
          </tr>
        </tbody>
      ...
      >
        <thead>
          ...
          <tr>
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '7% !important' : '140px'}"
            >
              返済期限
            </th>
            <th
            ...
            <th
              class="_table__light_header"
              :style="{ 'width': isPrint ? '9.375% !important' : '120px'}"
            >
              建単価<br>「現地通貨」
            </th>
            <th
        ...
        >
          <tr>
            ...
            <td class="_table__data">
              <span :class="[item.tradeTypeName === '売建' ? 'font-color__minus' : $_out(item.tradeTypeName) !== '-' && item.tradeTypeName === '買建' ? 'font-color__plus' : '', 'bold']">
                {{ $_out(item.tradeTypeName) }}
              </span><br>{{ $_out($_getCodeValue('MARGIN_DUE_DATE', 1, item.designationDealtClassification)) }}
            </td>
            <td class="_table__data">{{ $_getFormattedDateValue(item.lastTradeDate, 'date8') ? $_getFormattedDateValue(item.lastTradeDate, 'date8') : $_out(item.lastTradeDate) }}</td>
            <td class="_table__data _right">{{ $_out($_withCommaInteger(item.contractStandardDeposit)) }}</td>
            <td class="_table__data _right">{{ item.fxRate ? $_withCommaZeroPadding(item.fxRate) + '円/' + item.currency : '-' }}</td>
    ...
    >
      ...
      <table>
        <tr>
          ...
          <td>
            <span>ST/投資信託（SBIラップを含む）/先OP（保証金等）を除く上記の預り、価格は全てリアルタイムでの情報となります。</span>
          </td>
        </tr>
        <tr>
          <td style="width: 1.5rem; vertical-align: top;">・</td>
          <td>
            <span>投資信託（SBIラップ含む）、先OP（保証金等）については前営業日基準となります。</span>
          </td>
        </tr>
        <tr>
          <td style="width: 1.5rem; vertical-align: top;">・</td>
          <td>
            <span>STの参考単価は参考値となります。当該価格での買取を保証するものではございません。 </span>
          </td>
        </tr>
        <tr>
          <td style="width: 1.5rem; vertical-align: top;">・</td>
          <td>
            <span>上記の残高明細は、お客様のご参考のために作成され提供する暫定的かつ非公式なものです。残高について正式な詳細は、ＳＢＩ証券にお問い合わせいただき「取引残高報告書」等をご利用いただきますよう宜しくお願い申し上げます。</span>
          </td>
        </tr>
        <tr>
          <td style="width: 1.5rem; vertical-align: top;">・</td>
          <td>
            <span>資産状況に表示される現金(円貨)は、預り金、信用保証金、スィープ専用銀行口座の合計を表示します。</span>
          </td>
        </tr>
        <tr>
          <td style="width: 1.5rem; vertical-align: top;">・</td>
          <td>
            <span>資産状況に表示される現金(外貨)は、外貨預り金、米ドル信用保証金の合計を表示します。</span>
          </td>
        </tr>
        <tr>
          <td style="width: 1.5rem; vertical-align: top;">・</td>
          <td>
            <span>株式分割などのコーポレートアクションがある場合、権利付き最終日の引け後から権利落ち日の寄付きまでの間は、預かり残高や価格が正しく反映されない可能性があります。</span>
          </td>
        </tr>
      </table>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    mutualFunddepositTypeName(depositType, secTypeName) {
      if (secTypeName === 'SBIラップ投信') {
        return this.$_getCodeValue('ASSET_DEPOSIT_TYPE', 1, depositType)
      } else {
        return this.$_getCodeValue('ASSET_SPECIFIC_DEPOSIT_TYPE', 1, depositType)
      }
    }
  }
}
