...
COPY ifa-portal-backend/ifa-portal-gw/src ./ifa-portal-gw/src/

# プロジェクトビルド
RUN mvn -s ./settings.xml clean package -DskipTests -Pprod -B -pl ifa-portal-gw -am
...
# ビルド成果物コピー
COPY --chown=${IFA_UID}:${IFA_GID} --from=builder /app/ifa-portal-gw/target/ifa-portal-gw.jar ./helios-backend-gw.jar

# エントリーポイントコピー
COPY --chown=${IFA_UID}:${IFA_GID} --chmod=755 ci/entrypoint/gw-entrypoint.sh /app/entrypoint.sh

# ifauserユーザー切り替え
USER ${IFA_USER}

# ポート公開
EXPOSE ${GW_SERVER_PORT:-8080}
